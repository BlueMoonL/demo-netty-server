...
public class GpsTestClient {
    ...
    public void sendGpsData() {
        ...
        try {
            ...
            while (true) {
                ...
                gpsInfo.setDeviceId("TestDevice" + random.nextInt(1000));
    ...
    private PacketFrame createGpsPacket(GPSInfo gpsInfo) {
        ...
        // Header 생성
        HeaderFrame headerFrame = new HeaderFrame(HeaderFrame.STX, new byte[]{(byte) Opcode.GPS_INFO.getCode()}, ByteOrder.LITTLE_ENDIAN);
        headerFrame.setId(random.nextInt(1000)); // 랜덤 ID
        headerFrame.setDataLength(body.length);  // 데이터 바디 길이 설정

        // Tail 생성: Checksum 계산
        int checkSum = calculateCheckSum(headerFrame.createPacket(ByteOrder.LITTLE_ENDIAN), body);
        TailFrame tailFrame = new TailFrame(new byte[]{(byte) checkSum, 0x03}, ByteOrder.LITTLE_ENDIAN);
